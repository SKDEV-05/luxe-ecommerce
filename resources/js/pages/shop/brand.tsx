...
export default function BrandPage({ brand, products }: Props) {
    ...
    return (
        <StorefrontLayout>
            <Head title={`${brand.name} - Haute Maison`} />

            {/* Brand Banner */}
            <div className="relative bg-stone-950 text-stone-100 py-24 px-4 overflow-hidden border-b border-stone-900">
                {brand.logo_url && (
                    <div className="absolute inset-0 z-0 opacity-20">
                        <img 
                            src={brand.logo_url} 
                            alt={brand.name} 
                            className="w-full h-full object-cover filter grayscale sepia blur-xs"
                        />
                    </div>
                )}
                ...
                <div className="relative z-20 max-w-5xl mx-auto text-center space-y-4">
                    ...
                    <h1 className="text-4xl md:text-5xl font-serif font-bold tracking-widest uppercase text-amber-500 dark:text-amber-500">
                        {brand.name}
                    </h1>

                    {brand.country && (
                        <div className="inline-flex items-center gap-1 text-[10px] uppercase tracking-widest text-stone-400 font-mono">
                            <MapPin className="h-3.5 w-3.5 text-amber-600" /> Maison {brand.country}
                        </div>
                    )}
                    
                    {brand.description && (
                        <p className="text-sm md:text-base text-stone-300 font-serif max-w-xl mx-auto leading-relaxed pt-2">
                            {brand.description}
                        </p>
                    )}
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex items-center justify-between border-b border-stone-200 dark:border-neutral-900 pb-4 mb-8">
                    <span className="text-xs text-neutral-500 font-mono">
                        Collection of {products.total} exquisite products
                    </span>
                </div>
                ...
                {products.data.length > 0 ? (
                    <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {products.data.map((product) => (
                            <Card key={product.id} className="border-stone-200/50 dark:border-neutral-900 bg-white dark:bg-neutral-950 rounded-none overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full group">
                                <Link href={`/shop/${product.sku}`} className="relative aspect-[4/5] bg-stone-105 dark:bg-neutral-905 overflow-hidden block">
                                    <ProductImage
                                ...
                                <CardContent className="p-5 flex-grow flex flex-col justify-between space-y-4">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-1.5 text-xs text-neutral-455 font-serif">
                    ...
                    <div className="py-24 text-center text-neutral-400 space-y-4 border border-dashed border-stone-200 rounded-lg">
                        ...
                        <p className="text-sm max-w-xs mx-auto">There are currently no products available from {brand.name}.</p>
